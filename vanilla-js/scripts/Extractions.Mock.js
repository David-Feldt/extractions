...
/**
 * Adds a set of mock Shops to the Cloud Firestore.
 */
Extractions.prototype.addMockShops = function() {
  var promises = [];
  ...
  for (var i = 0; i < 20; i++) {
    ...
    if (!promise) {
      alert('addShop() is not implemented yet!');
...
/**
 * Adds a set of mock Ratings to the given shop.
 */
Extractions.prototype.addMockRatings = function(shopID) {
  var ratingPromises = [];
  for (var r = 0; r < 5*Math.random(); r++) {
    ...
    ratingPromises.push(this.addRating(shopID, rating));
  }
